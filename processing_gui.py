...
#feature to add:
#TODO add ability to remove the raw traces from the plot
...
class GUI(object):
    ...
    def widgets_plot_data_frame(self):
        ...
        raw_data_frame.pack(side=tk.TOP, fill=tk.X, pady=10)

        # Add the raw data plot check button
        self.raw_data_plot_check = ttk.Checkbutton(raw_data_frame, text='Plot Raw Data', variable=self.raw_data_plot_toggle)
        self.raw_data_plot_check.pack(side=tk.LEFT, padx=10)

        # Add the raw data label entry
        self.raw_data_label_entry = ttk.Entry(raw_data_frame, width=10)
        self.raw_data_label_entry.pack(side=tk.LEFT, padx=10)

        # Insert the default value for the raw data label
        self.raw_data_label_entry.insert(0, 'Raw Data')

        # Add the raw label check button
        self.raw_label_check = ttk.Checkbutton(raw_data_frame, text='Label toggle', variable=self.raw_label_toggle) 
        self.raw_label_check.pack(side=tk.LEFT, padx=10)
        ...
        self.processed_data_label_frame = ttk.Frame(self.plot_data_frame)
        self.processed_data_label_frame.pack(side=tk.TOP, fill=tk.X, pady=5)
    ...
    def update_plot(self):
        ...
        if ',' in self.index_entry_multiple.get():
            ...
            for sweep in self.temp_plot_traces:
                #if self.raw_data_plot_toggle is checked, plot the raw data with the label
                if self.raw_data_plot_toggle.get(): 
                    if self.raw_label_toggle.get():
                        ...
                        self.ax.plot(time, sweep, color='grey', linewidth=1, alpha=0.8)
                else:
                    #dont plot anything
                    pass
        ...
        if index >= 0 and ',' not in self.index_entry_multiple.get():
            ...
            #make sure remove label is not empty and if check button is checked, plot with the label
                if self.raw_label_toggle.get():
                    ...
                    self.ax.plot(time, data, color='grey', linewidth=1, alpha=0.8)
            else:
                pass
        ...
        if self.stim_removal_check.get():
            #if the stim_removal_check is checked and the label is checked plot the stim removed trace with the label
            if self.processed_label_toggle.get():
    ...
    def toggle_stim_removal(self):
        ...
        EPSP_list = ['EPSP_test_pulses', 'E_I_pulse']
        plateau_list = ['theta_burst', 'tetanus']

        if experiment_type in EPSP_list:
            stim_times = self.get_stim_times()
        ...
        if experiment_type in plateau_list:
        ...
        if 'stim_removed_trace' not in self.plot_check_buttons.keys():
            # Add the stim removed trace plot check button
            ...
            self.plot_check_buttons['stim_removed_trace'].pack(side=tk.LEFT, padx=10)
            ...
            self.processed_data_label['stim_removed_trace'].pack(side=tk.LEFT, padx=10)
            ...
            self.plot_labels_legend_list.append(self.processed_data_label['stim_removed_trace'])

            # Pack a check button for the processed data label
            self.label_stim_remove_check = ttk.Checkbutton(self.plot_data_frame, text='Stim Removed Label', variable=self.processed_label_toggle)
            self.label_stim_remove_check.pack(side=tk.LEFT, padx=10)
    ...
    def toggle_noise_removal(self):
        ...
        if 'noise_removed_trace' not in self.plot_check_buttons.keys():
            self.plot_check_buttons['noise_removed_trace'] = ttk.Checkbutton(self.plot_data_frame, text='noise_removed_trace', variable=self.noise_removal_check, command=self.toggle_noise_removal)
            self.plot_check_buttons['noise_removed_trace'].pack(side=tk.LEFT, padx=10)
            ...
            self.processed_data_label['noise_removed_trace'].pack(side=tk.LEFT, padx=10)

            #fill in the entry with the default label
            self.processed_data_label['noise_removed_trace'].insert(0, 'noise_removed_trace')

            #add label legend list
            self.plot_labels_legend_list.append('noise_removed_trace')

            #add a label check for the noise removed trace from the processed data label list
            self.label_noise_remove_check = ttk.Checkbutton(self.processed_data_label_frame, text='Noise Removed Label', variable=self.processed_label_toggle)
            self.label_noise_remove_check.pack(side=tk.LEFT, padx=10)
    ...
    def toggle_interpolated_spikes(self):
        ...
        if 'interpolated_spikes_trace' not in self.plot_check_buttons.keys():
            ...
            self.plot_check_buttons['interpolated_spikes_trace'].pack(side=tk.LEFT, padx=10)

            #create a entry for the processed data label
            self.processed_data_label['interpolated_spikes_trace'] = ttk.Entry(self.plot_data_frame, width=20)
            self.processed_data_label['interpolated_spikes_trace'].pack(side=tk.LEFT, padx=10)

            #fill in the entry with the default label
            self.processed_data_label['interpolated_spikes_trace'].insert(0, 'interpolated_spikes_trace')

            #add to label legend list
            self.plot_labels_legend_list.append('interpolated_spikes_trace')

            #add a label check for the noise removed trace from the processed data label list
            self.label_interpolated_spikes_check = ttk.Checkbutton(self.processed_data_label_frame, text='Interpolated Spikes Label', variable=self.processed_label_toggle)
            self.label_interpolated_spikes_check.pack(side=tk.LEFT, padx=10)
