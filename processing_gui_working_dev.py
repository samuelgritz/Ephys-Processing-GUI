...
def interp_spikes(AP_peak_window_start, AP_peak_window_end, spike_width, acquisition_frequency, voltage_trace, time):    
    ...
    for peak_idx in AP_peaks:
        ...
        try:
            ...
            print("Error in first derivative, trying second derivative")
            #otherwise try the second derivative
            derivative = np.diff(AP_trace, n=2)
            current_derivative = derivative.copy()
            derivative_filtered = signal.savgol_filter(current_derivative, 15, 3, mode='mirror')
            threshold_idx_in_AP_trace = find_peaks(derivative_filtered, height=0.5)[0][0] # Find peaks in the filtered derivative
...
def remove_artifacts_custom(data, artifact_times, acquisition_frequency, delete_start_stim, delete_end_stim):
    ...
    for channel in stim_times.keys():
        stim_times_list = stim_times[channel][0]
...
def remove_noise(data, noise_times, acquisition_frequency, delete_noise_duration_list):
    processed_data = np.copy(data)  # Make a copy of the data to avoid modifying the original
    noise_times = [noise_times] if type(noise_times) == int else noise_times
    delete_noise_duration_list = [delete_noise_duration_list] if type(delete_noise_duration_list) == int else delete_noise_duration_list
    # print('noise_times:', noise_times)
    # print('delete_noise_duration_list:', delete_noise_duration_list)

    if (len(noise_times) > 1) and (len(delete_noise_duration_list) > 1):
        for noise_time in noise_times:
            for delete_noise_duration in delete_noise_duration_list:
        ...
        noise_times = noise_times[0]
        delete_noise_duration = delete_noise_duration_list[0]
        current_noise_index = int(noise_times * acquisition_frequency / 1000)
        delete_start_index = max(0, current_noise_index - int(delete_noise_duration * acquisition_frequency / 1000))
        delete_end_index = min(len(processed_data), current_noise_index + int(delete_noise_duration * acquisition_frequency / 1000))
        current_window = np.arange(delete_start_index, delete_end_index)
        processed_data[current_window] = np.nan
...
class GUI(object):
    ...
    def widgets_analysis_steps_frame(self):
        ...
        self.stim_removal_automated_check_button = ttk.Checkbutton(self.analysis_steps_frame, text="Automated Stim Removal", variable=self.stim_removal_automated_check, command=self.toggle_stim_removal)
    ...
    def update_metadata(self):
        ...
        experiment_type = self.experiment_type_entry.get()
        ...
        if experiment_type:
            self.data_df.loc[self.current_index, 'stim_type'] = experiment_type
        else:
            self.data_df.loc[self.current_index, 'stim_type'] = None
            ...
            if self.num_pulses > 2:
                ...
                if self.ISI_time == 300:
                    ...
                    for channel in self.partition_data_dict:
                        stim_times_list = self.stim_times[channel][0]
                    ...
                    for channel in self.partition_data_dict:
                        ...
                        for stim_time in self.partition_data_dict[channel]:
                            ...
                            self.partition_time = np.linspace(partition_start_index, partition_end_index, len(self.partition_data_dict[channel][stim_time][0]))
                else:
                    for channel in self.partition_data_dict:
                        baseline_window_duration = self.defaults['analysis']['E_I_pulse'][channel]['baseline_window_duration']
                        baseline_window_duration = int(baseline_window_duration * acquisition_frequency / 1000)
